...
В `BookingHostedService` в методе обработки броней `ProcessBookingAsync` также используется `SemaphoreSlim`, т.к. в нем используются асинхронные методы сервиса `EventService` и `IBookingService`.
